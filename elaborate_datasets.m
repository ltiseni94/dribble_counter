...
for i = 1:length(label_list)
    ...
    label{i} = readtable(strcat(label_filename{i},'.csv'));
    pred{i} = readtable(strcat(pred_filename{i},'.csv'));
    traj{i} = readtable(strcat(traj_filename{i},'.csv'));
    fps{i} = readtable(strcat(fps_filename{i},'.csv'));
    dribble{i} = readtable(strcat(dribble_filename{i},'.csv'));
end
