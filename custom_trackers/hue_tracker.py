...
class HueTracker:
    def __init__(self, mean_h: int = 90, delta_h: int = 20, min_number_points: int = 15):
    ...
    def init(self, image: np.ndarray, bounding_box: List[int]):
        ...
        self.min_h = (self.mean_h - self.delta_h)/2
        self.max_h = (self.mean_h + self.delta_h)/2
    ...
    def update(self, image: np.ndarray):
        ...
        if self.min_h > self.max_h:
            mask = cv2.bitwise_not(cv2.inRange(bbox_new_hsv[:, :, 0], self.max_h, self.min_h))
        else:
            mask = cv2.inRange(bbox_new_hsv[:, :, 0], self.min_h, self.max_h)
